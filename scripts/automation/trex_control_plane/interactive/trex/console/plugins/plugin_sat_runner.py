...
class SatRunner_Plugin(ConsolePlugin):
    ...
    DEF_STA_IP_ADDR = '192.168.69.230'
    ...
    def __init__(self):
        ...
        self.console = None
        self.mixer_inited = False
        self.mixer_loadf = None
    ...
    def plugin_load(self):
        ...
        self.add_argument('--atten-value', action='store', nargs="?", default=-1, type=int, required=False,
                          dest="atten_value",
    ...
    def build_xlsx(self, filename):
        ...
        for angle, points in keep_angle_scan_atten.items():
            ...
            for atten in self.atten_range:
                ...
                if keep_angle_scan_atten[angle].get(atten) == None:
                    keep_angle_scan_atten[angle].update(
                        {atten: [[0, 0, 0], [0, -127]]})
            # build xlsx
            ...
            for atten, point in points.items():
                ...
                if point_idx == 0:
                    keep_atten_scan_angle[atten].update(
                        {angle: "='DataRaw'!$B${:d}".format(offset + len(atten_col) - 1)})
                else:
                    keep_atten_scan_angle[atten].update(
                        {angle: ",'DataRaw'!$B${:d}".format(offset + len(atten_col) - 1)})
            ...
            sheet_dataraw.write_column(
                'D{:d}'.format(offset), link_quality_col)

            ds_atten = "='DataRaw'!$A${:d}:$A${:d}".format(
                offset + 1, offset + len(atten_col) - 1)
            ds_line_thoughput = "='DataRaw'!$B${:d}:$B${:d}".format(
                offset + 1, offset + len(throughput_col) - 1)
            ds_line_rssi = "='DataRaw'!$C${:d}:$C${:d}".format(
                offset + 1, offset + len(signal_lvl_col) - 1)
        ...
        ''' =Dashboard!$A$2:$A$8 '''
        ds_angles = "='Dashboard'!$A${:d}:$A${:d}".format(
            offset + 1, offset + len(angles))
    ...
    def init_atten_group(self):
        ...
        if atten == self.USE_ATTEN_ADAURA:
            ...
            print("Not supported attenuator: {}, use default {}".format(
                atten, self.USE_ATTEN_ADAURA))
    ...
    def do_setup(self, atten_selection, sta_addr, sta_passwd, rota_angles, atten_value):
        ...
        if type(rota_angles) == list and len(rota_angles) > 0 and rota_angles[0] >= 0 and rota_angles[0] <= 14:
            self.rotate.set_value(rota_angles[0])
        elif rota_angles >= 0 and rota_angles <= 14:
            self.rotate.set_value(rota_angles)
        else:
            print("error rota angle value {}\n".format(rota_angles), color='red')

    # We build argparser from do_* functions, stripping the "do_" from name
    # <------ name was registered in plugin_load
    def do_report(self, dir_report, file_report):
    ...
    def gen_atten_prediction(self, mbps):
        ...
        if ratio == 0:
            ...
            self.time_fraction = int(
                self.time_fraction_passed * len(self.rota_angles) / (self.rota_angle_idx + 1))
            return

        if self.rota_angle_idx > 0:
            if self.atten_range_len + 1 < len(self.atten_range):
                print("Prediction {} Mb, cached {} of {}".format(
                    mbps, self.atten_range_len, self.atten_range))
        ...
        print("Prediction {}/{} Mb, new {}->{}".format(mbps,
              stop_mbps, atten_current, atten_prediction))
    ...
    def update_processbar(self):
        ...
        time_needed = int(time_passed * self.time_fraction /
                          self.time_fraction_passed)
    ...
    def gen_report(self, finised=True):
        ...
        if finised == True:
            partitial = ""
        report_name = self.build_xlsx("{0}-{1}-{2}-{3}.xlsx".format(
            self.test_prefix + partitial, self.update_ts_subfix(), self.subfix_mode, self.subfix_rota))
        self.beep_long()
    ...
    def init_angles(self, rota_angles):
        ...
        if type(rota_angles) == list:
            ...
            self.subfix_rota = "Rota-{}-{}-{}".format(
                len(rota_angles), rota_angles[0], rota_angles[-1])
        ...
        print("Rotar angles:{}, time fract {}".format(
            self.rota_angles, self.time_fraction), color='green')
    ...
    def do_prepare(self, atten_start, atten_step, atten_stop, continuous, atten_value, precision, time_intval, rota_angles, auto_report, test_prefix):
        ...
        if continuous == 0:
            ...
            time_fraction = int((atten_stop - atten_start) /
                                atten_step) * self.precision
            self.subfix_mode = "Atten-{}-{}-{}-{}".format(
                atten_start, atten_step, atten_stop, time_intval)
            print("Test and report, atten from {0} step {1} to {2}".format(
                atten_start, atten_step, atten_stop))
            print("Target: {0} cut to 5*X = {1}".format(atten_stop,
                  int(atten_stop / 5) * 5), color='red')
            print("  Angle {}: {}".format(
                type(rota_angles), rota_angles), color='green')
            print("  Atten group: from {:d} to {:d} step: {:d} intv: {:d} sec, {:d}.\n".format(
                atten_start, atten_stop, atten_step, time_intval, time_fraction))
        else:
            self.atten_range_len = 1
            ...
            print("Run continuous scan: {:d} secs, {:d}".format(
                continuous, time_fraction), color='green', format='bold')
    ...
    def do_status(self):
        ...
        if self.ts_start != None and self.finished != True:
            ...
            print("Idle and prepared {}, finished {}".format(
                self.prepared, self.finished), color='green', format='bold')
    ...
    def do_run_next(self) -> int:
        ...
        self.atten_samples.update(
            {self.atten_base_value + atten_value: [rx_mbps[:], self.update_rssi()]})
    ...
    def do_unit_test_report(self):
        ...
        self.update_ts_subfix()
        self.build_xlsx(
            "unitest-{0}-{1}.xlsx".format("report", self.subfix_time))
    ...
    def echo(self, fname):
        ...
        try:
            ...
            if self.mixer_inited != True:
                ...
                mixer.init()
            if self.mixer_loadf != fname:
                # print("load...")
                self.mixer_loadf = fname
